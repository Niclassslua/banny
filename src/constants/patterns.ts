...
const addAlpha = (hex: string, alpha: string): string => {
    if (!hex.startsWith("#")) {
        return hex;
    }
...
export const patterns: Pattern[] = [
    ...
    },
    {
        name: "Indigo Lattice",
        style: (scale: number, color1: string, color2: string) => {
            const size = Math.max(scale * 4, 16);
            return `
      background: linear-gradient(45deg, ${color1} 10%, transparent 11%, transparent 89%, ${color1} 90%),
                  linear-gradient(135deg, ${color1} 10%, transparent 11%, transparent 89%, ${color1} 90%),
                  radial-gradient(circle, transparent 25%, ${color2} 26%),
                  linear-gradient(45deg, transparent 46%, ${color1} 47%, ${color1} 52%, transparent 53%),
                  linear-gradient(135deg, transparent 46%, ${color1} 47%, ${color1} 52%, transparent 53%);
      background-size: ${size}px ${size}px;
      background-color: ${color2};
      opacity: 1.0;
    `;
    ...
    },
    {
        ...
        style: (scale: number, color1: string, color2: string) => {
            const size = Math.max(scale * 4, 16);
            return `
      background: radial-gradient(circle at top, transparent 14%, ${color1} 15%, ${color1} 19%, transparent 20%),
                  radial-gradient(circle at bottom, transparent 14%, ${color1} 15%, ${color1} 19%, transparent 20%),
                  radial-gradient(circle at right, transparent 14%, ${color1} 15%, ${color1} 19%, transparent 20%),
                  radial-gradient(circle at left, transparent 14%, ${color1} 15%, ${color1} 19%, transparent 20%),
                  radial-gradient(circle at center, ${color1} 25%, transparent 26%),
                  radial-gradient(circle at bottom left, ${color1} 12%, transparent 13%),
                  radial-gradient(circle at bottom right, ${color1} 12%, transparent 13%),
                  radial-gradient(circle at top left, ${color1} 12%, transparent 13%),
                  radial-gradient(circle at top right, ${color1} 12%, transparent 13%);
      background-size: ${size}px ${size}px;
      background-color: ${color2};
      opacity: 1.0;
    `;
    ...
    },
    {
        ...
        style: (scale: number, color1: string, color2: string) => {
            const size = Math.max(scale * 4, 16);
            return `
      background: radial-gradient(circle at top left, transparent 9%, ${color1} 10%, ${color1} 15%, transparent 16%),
                  radial-gradient(circle at bottom left, transparent 9%, ${color1} 10%, ${color1} 15%, transparent 16%),
                  radial-gradient(circle at top right, transparent 9%, ${color1} 10%, ${color1} 15%, transparent 16%),
                  radial-gradient(circle at bottom right, transparent 9%, ${color1} 10%, ${color1} 15%, transparent 16%),
                  radial-gradient(circle, transparent 25%, ${color2} 26%),
                  linear-gradient(45deg, transparent 46%, ${color1} 47%, ${color1} 52%, transparent 53%),
                  linear-gradient(135deg, transparent 46%, ${color1} 47%, ${color1} 52%, transparent 53%);
      background-size: ${size}px ${size}px;
    ...
    },
    {
        ...
        style: (scale: number, color1: string, color2: string) => {
            const size = Math.max(scale * 4, 16);
            return `
      background: radial-gradient(circle at bottom left, ${color1} 35%, transparent 36%),
                  radial-gradient(circle at top right, ${color1} 35%, transparent 36%),
                  radial-gradient(circle at center, ${color1} 15%, transparent 16%);
      background-size: ${size}px ${size}px;
      background-color: ${color2};
      opacity: 1.0;
    `;
    ...
    {
        name: "Criss Cross Bloom",
        style: (scale: number, color1: string, color2: string) => {
            const size = Math.max(scale * 4, 16);
            return `
      background: linear-gradient(45deg, transparent 14%, ${color1} 15%, ${color1} 20%, transparent 21%, transparent 79%, ${color1} 80%, ${color1} 85%, transparent 86%),
                  linear-gradient(135deg, transparent 14%, ${color1} 15%, ${color1} 20%, transparent 21%, transparent 79%, ${color1} 80%, ${color1} 85%, transparent 86%),
                  radial-gradient(transparent 14%, ${color1} 15%, ${color1} 25%, transparent 26%);
      background-size: ${size}px ${size}px;
      background-color: ${color2};
      opacity: 1.0;
    `;
    ...
    },
    {
        ...
        style: (scale: number, color1: string, color2: string) => {
            const size = Math.max(scale * 4, 16);
            const accent99 = addAlpha(color1, "99");
            return `
      background: linear-gradient(-45deg, ${accent99} 20%, transparent 20% 80%, ${accent99} 80% 100%),
                  linear-gradient(45deg, ${accent99} 20%, transparent 20% 80%, ${accent99} 80% 100%),
                  linear-gradient(-45deg, ${color2} 33%, transparent 33% 66%, ${color2} 66%),
                  linear-gradient(45deg, transparent 33%, ${color1} 33% 66%, transparent 66%);
      background-size: ${size}px ${size}px;
      background-color: ${color2};
      opacity: 1.0;
    `;
    ...
    },
    {
        ...
        style: (scale: number, color1: string, color2: string) => {
            const size = Math.max(scale * 4, 16);
            return `
      background: radial-gradient(transparent 79%, ${color1} 80%);
      background-size: ${size}px ${size}px;
      background-color: ${color2};
      opacity: 1.0;
    `;
    ...
    },
    {
        ...
        style: (scale: number, color1: string, color2: string) => {
            const size = Math.max(scale * 4, 16);
            return `
      background: linear-gradient(45deg, transparent 49%, ${color1} 49% 51%, transparent 51%),
                  linear-gradient(-45deg, transparent 49%, ${color1} 49% 51%, transparent 51%);
      background-size: ${size}px ${size}px;
      background-color: ${color2};
      opacity: 1.0;
    `;
    ...
    },
    {
        ...
        style: (scale: number, color1: string, color2: string) => {
            const size = Math.max(scale * 4, 16);
            return `
      background: repeating-radial-gradient(circle, ${color1}, transparent 20%);
      background-size: ${size}px ${size}px;
      background-color: ${color2};
      opacity: 1.0;
    `;
    ...
    },
    {
        ...
        style: (scale: number, color1: string, color2: string) => {
            const size = Math.max(scale * 4, 16);
            return `
      background: linear-gradient(45deg, ${color1} 0%, ${color1} 5%, transparent 5%, transparent 10%, ${color1} 10%, ${color1} 15%, transparent 15%, transparent 20%, ${color1} 20%, ${color1} 25%, transparent 25%, transparent 30%, ${color1} 30%, ${color1} 35%, transparent 35%, transparent 40%, ${color1} 40%, ${color1} 45%, transparent 45%, transparent 50%, ${color1} 50%, ${color1} 55%, transparent 55%, transparent 60%, ${color1} 60%, ${color1} 65%, transparent 65%, transparent 70%, ${color1} 70%, ${color1} 75%, transparent 75%, transparent 80%, ${color1} 80%, ${color1} 85%, transparent 85%, transparent 90%, ${color1} 90%, ${color1} 95%, transparent 95%),
                  linear-gradient(135deg, ${color1} 0%, ${color1} 5%, transparent 5%, transparent 10%, ${color1} 10%, ${color1} 15%, transparent 15%, transparent 20%, ${color1} 20%, ${color1} 25%, transparent 25%, transparent 30%, ${color1} 30%, ${color1} 35%, transparent 35%, transparent 40%, ${color1} 40%, ${color1} 45%, transparent 45%, transparent 50%, ${color1} 50%, ${color1} 55%, transparent 55%, transparent 60%, ${color1} 60%, ${color1} 65%, transparent 65%, transparent 70%, ${color1} 70%, ${color1} 75%, transparent 75%, transparent 80%, ${color1} 80%, ${color1} 85%, transparent 85%, transparent 90%, ${color1} 90%, ${color1} 95%, transparent 95%);
      background-size: ${size}px ${size}px;
      background-color: ${color2};
      opacity: 1.0;
    `;
    ...
    {
        name: "Layered Diamonds",
        style: (scale: number, color1: string, color2: string) => {
            const size = Math.max(scale * 4, 16);
            return `
      background: linear-gradient(45deg, ${color2} 20%, transparent 20%, transparent 80%, ${color2} 80%),
                  linear-gradient(135deg, ${color2} 40%, ${color1} 40%, ${color1} 60%, ${color2} 60%),
                  linear-gradient(45deg, ${color2} 20%, ${color1} 20%, ${color1} 80%, ${color2} 80%);
      background-size: ${size}px ${size}px;
      background-color: ${color2};
      opacity: 1.0;
    `;
    ...
    },
    {
        ...
        style: (scale: number, color1: string, color2: string) => {
            const size = Math.max(scale * 4, 16);
            const accent80 = addAlpha(color1, "80");
            return `
      background: radial-gradient(circle at left bottom, transparent 14%, ${color1} 15%, ${color1} 25%, transparent 26%, transparent 34%, ${accent80} 35%, ${accent80} 45%, transparent 46%, transparent 54%, ${color1} 55%, ${color1} 65%, transparent 66%, transparent 74%, ${accent80} 75%, ${accent80} 85%, transparent 86%);
      background-size: ${size}px ${size}px;
      background-color: ${color2};
      opacity: 1.0;
    `;
    ...
    },
    {
        ...
        style: (scale: number, color1: string, color2: string) => {
            ...
            const accent80 = addAlpha(color1, "80");
            return `
      background: radial-gradient(circle at 40% 40%, ${color1} 25%, transparent 26%),
                  radial-gradient(circle at 60% 60%, ${accent80} 25%, transparent 26%);
      background-size: ${size}px ${size}px;
      background-color: ${color2};
      opacity: 1.0;
    `;
    ...
    },
    {
        ...
        style: (scale: number, color1: string, color2: string) => {
            const size = Math.max(scale * 4, 16);
            return `
      background: radial-gradient(circle at 25% 25%, transparent 14%, ${color1} 15%, ${color1} 20%, transparent 21%),
                  radial-gradient(circle at 75% 75%, transparent 11%, ${color1} 12%, ${color1} 17%, transparent 18%);
      background-size: ${size}px ${size}px;
      background-color: ${color2};
      opacity: 1.0;
    `;
    ...
    },
    {
        ...
        style: (scale: number, color1: string, color2: string) => {
            const size = Math.max(scale * 4, 16);
            return `
      background: radial-gradient(circle at 25% 25%, ${color1} 20%, transparent 21%),
                  radial-gradient(circle at 75% 75%, ${color1} 15%, transparent 16%);
      background-size: ${size}px ${size}px;
      background-color: ${color2};
      opacity: 1.0;
    `;
    ...
    },
    {
        ...
        style: (scale: number, color1: string, color2: string) => {
            const size = Math.max(scale * 4, 16);
            return `
      background: radial-gradient(circle at bottom left, transparent 25%, ${color1} 25.5%, ${color1} 36%, transparent 37%),
                  radial-gradient(circle at top right, transparent 34%, ${color1} 34.5%, ${color1} 45.5%, transparent 46%);
      background-size: ${size}px ${size}px;
      background-color: ${color2};
      opacity: 1.0;
    `;
